...
const SCHEDULE = process.env.TASK_REMINDER_CRON ?? "*/1 * * * *"; // Default: every 5 minutes for testing";
...
const globalStateKey = "__taskReminderSchedulerState";
...
export function startTaskReminderScheduler() {
  ...
  console.log(
    `[TaskReminder] Scheduler aktif: '${SCHEDULE}' timezone=${TIMEZONE}`,
  );
}
