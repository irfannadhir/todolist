...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    return Response.json({ message: "Terjadi kesalahan saat mengubah user" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (session.sub === id) {
      return Response.json({ message: "User login aktif tidak bisa dihapus" }, { status: 400 });
    }
    ...
    return Response.json({ message: "Terjadi kesalahan saat menghapus user" }, { status: 500 });
  }
}
